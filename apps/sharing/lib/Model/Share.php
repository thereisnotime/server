	...
	public function __construct(
		...
		foreach ($properties as $featureClass => $featureProperties) {
			...
			if (array_intersect($registry->getSourceTypesCompatibleWithFeature($featureClass), $shareSourceTypes) === []) {
				throw new ShareInvalidException('The feature is not compatible with any of the source types of the share: ' . var_export($featureClass, true));
			}

			if (array_intersect($registry->getRecipientTypesCompatibleWithFeature($featureClass), $shareRecipientTypes) === []) {
				throw new ShareInvalidException('The feature is not compatible with any of the recipient types of the share: ' . var_export($featureClass, true));
			}
